...
export const Card: React.FC<{
  ...
  const thumbnail = heroImage?.sizes?.small?.url || heroImage?.sizes?.thumbnail?.url
        ...
        {titleToUse && (
          <>
            <h3 className="text-lg font-semibold ">
              <Link className="not-prose text-primary font-normal" href={href} ref={link.ref}>
                {titleToUse}
              </Link>
            </h3>
            ...
            <div className="flex py-2 flex-row gap-2 text-[0.8em] items-center">
              ...
              {publishedAtFromDoc && (
                <p className="text-muted-foreground">{formatDatePretty(publishedAtFromDoc)}</p>
              )}
            </div>
          </>
        )}
