...
export const PostHero: React.FC<{
  ...
  const { categories, heroImage, populatedAuthors, publishedAt, title, subtitel } = post
    ...
    <div className="container flex flex-col items-center min-h-96 ">
      <div className="w-full lg:grid lg:grid-cols-[1fr_48rem_1fr] py-4">
        <div className="col-start-2 col-span-[48rem] flex flex-col items-center gap-6">
          {publishedAt && (
            <p className="text-xs text-muted-foreground">{formatDateFull(publishedAt)}</p>
          )}
          <div className="flex flex-col gap-2">
            <h1 className="effect-font-hero effect-font-gradient font-medium text-2xl md:text-5xl">
              {title}
            </h1>
            <h3 className="text-lg font-light text-muted-foreground">{subtitel}</h3>
          </div>
          {hasAuthors && (
            <div className="flex flex-row gap-2 items-center">
              ...
              <p className="text-sm text-muted-foreground">{formatAuthors(populatedAuthors)}</p>
            </div>
          )}
        </div>
      ...
      <div className="w-4xl max-w-4xl border border-border rounded-xl overflow-hidden mb-12">
        {heroImage && typeof heroImage !== 'string' && (
          <Media priority imgClassName="-z-10" className="max-w-4xl" resource={heroImage} />
        )}
